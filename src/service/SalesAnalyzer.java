...
//        cal.add(Calendar.DAY_OF_MONTH, -30); // Lùi 30 ngày
...
//    	    DishDAO.loadData(); // Gọi hàm load dữ liệu
//    	}
//    	
//        // Duyệt qua từng món trong top 7 để tính doanh thu
//        for (String dishID : topDishes) {
//        	
//            Dish dish = DishDAO.map.get(dishID);
//            if (dish == null) continue; // Nếu không tìm thấy món, bỏ qua
//
//            double revenueToday = 0;
//            double revenueLast30Days = 0;
//            int daysWithSales = 0;
//
//
...
//        	    BillDAO.loadData(); // Gọi hàm load dữ liệu
//        	}
//        	
//            // Duyệt qua tất cả hóa đơn để tính doanh thu theo từng khoảng thời gian
//            for (Bill bill : BillDAO.list) {
//            	
//                Date billDate = bill.getTime();
//                
//                if (billDate.after(last30Days) && billDate.before(today)) {
//                    // Hóa đơn trong 30 ngày gần nhất
//                    for (DetailReceipt detail : bill.getDetailList()) {
//                        if (detail.getDishID().equals(dishID)) {
//                            revenueLast30Days += detail.getDishQuantity() * dish.getDishPrice();
//                        }
//                    }
//                    daysWithSales++;
//                } else if (isSameDay(billDate, today)) { 
//                	System.out.println("Hóa đơn hôm nay: " + bill.getBillID());
//                    // Hóa đơn của ngày hôm nay
//                    for (DetailReceipt detail : bill.getDetailList()) {
//                        if (detail.getDishID().equals(dishID)) {
//                            revenueToday += detail.getDishQuantity() * dish.getDishPrice();
//                            System.out.println("Bill Time: " + bill.getTime());
//                        }
//                    }
//                }
//            }
//
//            // Tính doanh số trung bình tháng (chia cho số ngày có doanh số)
//            double avgMonthlyRevenue = (daysWithSales > 0) ? (revenueLast30Days / daysWithSales) : 0;
//
...
//    }
    ...
    public static List<SalesData> getTop10BestSellingDishes() {
        ...
        for (String dishID : topDishes) {
            ...
            System.out.println(revenueToday + " " + revenueLast30Days + " " + daysWithSales);
            System.out.println(dish.getDishName() + " " + avgMonthlyRevenue);
